...
const EditCareerRequestModal = ({ isOpen, onClose, request, onSuccess }) => {
  ...
  const statusOptions = ['new', 'reviewed', 'contacted', 'rejected'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestRef = doc(db, 'careersfromwebsite', request.id);
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-3 sm:p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Edit Career Request</h2>
          <button
            onClick={onClose}
...
export default EditCareerRequestModal;
