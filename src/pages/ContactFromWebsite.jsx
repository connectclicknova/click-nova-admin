...
const ContactFromWebsite = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'contactsfromwebsite'), (snapshot) => {
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'contactsfromwebsite', editingContact.id), formData);
      showToast('Contact updated successfully', 'success');
      setIsEditModalOpen(false);
      setEditingContact(null);
      setFormData({ name: '', mobile: '', city: '', service: '', status: 'new' });
    } catch (error) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await updateDoc(doc(db, 'contactsfromwebsite', id), { status: newStatus });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this contact?')) {
      try {
        await deleteDoc(doc(db, 'contactsfromwebsite', id));
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 sm:mb-8 gap-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Contact From Website</h1>
      </div>
              ...
              {fetchingData ? (
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-gray-500">
                    {searchTerm || statusFilter !== 'All' ? 'No contacts match your search criteria.' : 'No contacts found.'}
                  </td>
                </tr>
              ) : (
                filteredContacts.map((contact) => (
                  <tr key={contact.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{contact.service}</td>
              ...
              <form onSubmit={handleUpdate}>
                ...
                <div className="flex gap-3">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:bg-blue-300 cursor-pointer flex items-center justify-center gap-2"
                  >
...
export default ContactFromWebsite;
